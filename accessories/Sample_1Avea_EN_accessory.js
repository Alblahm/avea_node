...
var OFFICELIGHT = {
  ...
  setHue: function(hue){
    //console.log("... Setting Hue value to %s", hue);
    OFFICELIGHT.hue = hue;
    OFFICELIGHT.bChangeSth = true;
    OFFICELIGHT.sendToLight(true);
  },
  setSaturation: function(saturation){
    //console.log("... Setting saturation value to %s", saturation);
  ...
  setPowerOn: function(onValue) { 
    ...
    if((onValue==false)||(OFFICELIGHT.bChangeSth == false)){
    	//Update the device state with the actual values...
  ...
  sendToLight: function(posValue){
    ...
    if(posValue==true){
      //console.log("... Turning on %s!",txtIdLamp);
      //console.log("... Sending command to the light");
      ...
      //console.log("... Turning off %s!", txtIdLamp);
...
light.username = "C7:1A:3E:1C:5A:AD";
...
light
  .addService(Service.Lightbulb, txtSiriName) // services exposed to the user should have "names" like "Fake Light" for us
  ...
  .on('set', function(value, callback) {
	...
	// Now it changes if the BTLe is connected
	if((perifSel.state == "connected") && (bulb.connected==true)){
	   ...
	   callback();
        ...
        console.log("... The light is not yet reachable or it is not connected!!");
...
light
  ...
  .on('get', function(callback) {
    ...
    // Now it changes if the BTLe is connected
    if(perifSel!=null){
      // The data value returned by the Prosise is read, it should contain something like: 
      ...
      if((perifSel.state == "connected") && (bulb.connected==true)){
	   ...
	   if (bCheckColor == true){
		console.log("... Was " + txtIdLamp + " turned on? No.");
		OFFICELIGHT.powerOn = false;
		callback(err, false);
	   }else{
		console.log("... Was " + txtIdLamp + "turned on? Yes.");
  ...
  if(perifSel==null){
	...
	//console.log("... Dispositivo identificado");
        ...
        perifSel.connect(function(error) {
       	  //console.log('... (Init) connecting the device: ' + perifSel.uuid);
       	  bulb = new avea.Avea(perifSel);
       	  bulb.connect();
       	  console.log("... (Init) Perif: " + perifSel.state + " / Luz: " + bulb.connected + ", UUID:" + perifSel.uuid);
...
noble.on('stateChange', function(state) {
  ...
  //console.log("... NOBLE service status: %s", state);
  if (state === 'poweredOn') {
      	noble.startScanning(['f815e810456c6761746f4d756e696368'], false);
  } else {
        noble.stopScanning();
...
//           Command 3(...): "*****"
// --------------------------------------------------
//       Command 4(english): change the color of the living room lamp to red / green / orange / pink / red
//       Command 4(spanish): Cambia el color de la Lámpara de Pie a azul / verde / naranja / rosa / rojo
